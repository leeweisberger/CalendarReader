...
public class RectangleBounder {
	...
	public static List<Rect> getRectangles(List<MatOfPoint> contours, double widthDivHeight, int contourAreaThresh, int minWidth, int minHeight) {
		...
		for (int x = 0; x < contours.size(); x++) {
			...
			if(area>contourAreaThresh){
				...
				//only get rectangles that pass the ratio, height, and width checks
				if((1.0*r.width/r.height)<widthDivHeight && r.height>minHeight && r.width>minWidth && r.height<MAX_HEIGHT){
					bufferRectangle(r);
	...
	public static void drawRectangles(Mat m, List<Rect> rectangles) {
		...
		for(Rect r : rectangles){
			...
			Core.rectangle(m, r.tl(), r.br(), ImageProcessingUtils.SCALARS[new Random().nextInt(ImageProcessingUtils.SCALARS.length)],ImageProcessingUtils.DRAW_WIDTH);
		}
